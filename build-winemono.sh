...
cross_build_mono ()
{
	...
	make $MAKEOPTS "CC=${MINGW}-gcc" -f "$SRCDIR/Theorafile/Makefile" || exit 1
    cp "$BUILDDIR/build-cross-$ARCH/Theorafile/libtheorafile.dll" "$BUILDDIR/image/bin/libtheorafile-$ARCH.dll" || exit 1
}
